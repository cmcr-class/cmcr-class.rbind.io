...
These names won't win any awards, but they won't cause errors, so the `janitor` totally succeeded in cleaning up.  It's a great option for taking care of big messes fast.  At the other end of the scale, if you only need to fixing just a few problematic names, is to use `rename()`'s new(-ish) assign-by-position ability:
...
An intermediate option is to assign a complete vector of names:
